// =====================================================
// TASK TRACKING UI - ID-BASED FRONTEND INTERFACE
...
class TaskTrackingUI {
    ...
    canEditTask(task) {
        ...
        if (this.userContext.position === 'Coordinator' && task.assigned_to_member?.team_name === this.userContext.team_name) return true;
        
        // Member can edit only assigned tasks
        if (this.userContext.position === 'Member' && task.assigned_to === this.userContext.id) return true;
    ...
    switchTab(tab) {
        ...
        switch (tab) {
            ...
            case 'my-tasks':
                ...
                document.getElementById('filtersSection').classList.remove('hidden');
                this.currentFilters = { assigned_to_id: this.userContext.id };
                this.loadMyTasks();
                break;
    ...
    async loadMyTasks() {
        const result = await this.taskTracker.getTasks({ assigned_to_id: this.userContext.id });
        if (result.success) {
            this.tasks = result.data;
    ...
    async createTask() {
        const taskData = {
            ...
            assigned_to: parseInt(document.getElementById('taskAssignTo').value), // This is now an ID
