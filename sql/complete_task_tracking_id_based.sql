-- =====================================================
-- COMPLETE ID-BASED TASK TRACKING SYSTEM
...
-- =====================================================
-- STEP 1: PREPARE prm_members TABLE
-- =====================================================

-- Create indexes on prm_members for better JOIN performance
CREATE INDEX IF NOT EXISTS idx_prm_members_id ON prm_members(id);
CREATE INDEX IF NOT EXISTS idx_prm_members_email ON prm_members(email);
CREATE INDEX IF NOT EXISTS idx_prm_members_team_name ON prm_members(team_name);
CREATE INDEX IF NOT EXISTS idx_prm_members_position ON prm_members(position);
...
-- =====================================================
-- STEP 3: CREATE ID-BASED TABLES
-- =====================================================

-- Main tasks table (ID-based foreign keys)
CREATE TABLE tasks (
    id BIGSERIAL PRIMARY KEY,
    title VARCHAR(255) NOT NULL CHECK (LENGTH(TRIM(title)) > 0),
    description TEXT,
    assigned_by BIGINT NOT NULL REFERENCES prm_members(id) ON DELETE RESTRICT,
    assigned_to BIGINT NOT NULL REFERENCES prm_members(id) ON DELETE RESTRICT,
...
-- Task comments (ID-based)
CREATE TABLE task_comments (
    id BIGSERIAL PRIMARY KEY,
    task_id BIGINT NOT NULL REFERENCES tasks(id) ON DELETE CASCADE,
    member_id BIGINT NOT NULL REFERENCES prm_members(id) ON DELETE RESTRICT,
    comment TEXT NOT NULL CHECK (LENGTH(TRIM(comment)) > 0),
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);

-- Task attachments (ID-based)
CREATE TABLE task_attachments (
    id BIGSERIAL PRIMARY KEY,
    task_id BIGINT NOT NULL REFERENCES tasks(id) ON DELETE CASCADE,
    uploaded_by BIGINT NOT NULL REFERENCES prm_members(id) ON DELETE RESTRICT,
...
-- Primary performance indexes for ID-based FKs
CREATE INDEX idx_tasks_assigned_to_id ON tasks(assigned_to);
CREATE INDEX idx_tasks_assigned_by_id ON tasks(assigned_by);
...
CREATE INDEX idx_task_comments_member_id ON task_comments(member_id);
...
BEGIN
    ...
    -- Get user ID from prm_members using email
...
DECLARE
    ...
    user_id BIGINT;
BEGIN
    SELECT get_user_id() INTO user_id;
    
    -- Get position from prm_members using ID
...
BEGIN
    ...
    -- Get team from prm_members using ID
...
-- Check if user can edit task (ID-based)
...
DECLARE
    ...
    task_status TEXT;
BEGIN
    SELECT get_user_role(), get_user_team(), get_user_id() 
    INTO user_role, user_team, user_id;
    
    SELECT t.assigned_to, m.team_name, t.status 
    INTO task_assignee_id, task_assignee_team, task_status
    FROM tasks t
    JOIN prm_members m ON t.assigned_to = m.id
    WHERE t.id = task_id;
    ...
    IF user_role = 'Member' AND task_assignee_id = user_id THEN
        RETURN TRUE;
    END IF;
...
-- Check if user can view task (ID-based)
...
-- Function to validate team assignment (ID-based)
...
BEGIN
    -- Get assignee team using ID
    SELECT team_name INTO assignee_team 
    FROM prm_members 
    WHERE id = NEW.assigned_to;
    
    -- Get assigner team and position using ID
    ...
    IF assignee_team IS NULL THEN
        RAISE EXCEPTION 'Assignee ID % not found in members table', NEW.assigned_to;
    END IF;
    
    -- Check if assigner exists
    IF assigner_team IS NULL THEN
        RAISE EXCEPTION 'Assigner ID % not found in members table', NEW.assigned_by;
    END IF;
...
-- COORDINATOR POLICIES: Manage tasks in their team (ID-based)
CREATE POLICY "coordinator_team_tasks" ON tasks
    ...
    USING (
        ...
        AND EXISTS (
            SELECT 1 FROM prm_members assignee
...
-- MEMBER POLICIES: View all, update only assigned tasks (ID-based)
...
-- Team Performance View (ID-based joins)
...
-- Member Performance View (ID-based with full member info)
...
GROUP BY m.id, m.email, m.name, m.team_name, m.position, m.phone, m.scout_group, m.district, m.stage
ORDER BY member_score DESC, total_points_earned DESC;

-- Coordinator Performance View (ID-based)
...
GROUP BY m.id, m.email, m.name, m.team_name, m.phone, m.scout_group, m.district
ORDER BY coordinator_effectiveness_score DESC, assignment_success_rate DESC;

-- Dashboard Summary View (ID-based)
...
SELECT 
    ...
    -- Team statistics
    (SELECT COUNT(DISTINCT assignee.team_name) FROM tasks t JOIN prm_members assignee ON t.assigned_to = assignee.id) as active_teams,
    (SELECT COUNT(DISTINCT assigned_to) FROM tasks) as active_members,
    
    -- Best performers (ID-based)
...
COMMENT ON TABLE tasks IS 'Main task tracking table with ID-based foreign keys to prm_members';
...
COMMENT ON COLUMN tasks.assigned_by IS 'ID of the person who assigned the task (from prm_members)';
COMMENT ON COLUMN tasks.assigned_to IS 'ID of the person assigned to the task (from prm_members)';
...
SELECT 
    '🎉 ID-BASED TASK TRACKING SYSTEM SETUP COMPLETE! 🎉' as status,
    NOW() as completed_at,
    'System is ready for use with ID-based foreign keys to prm_members table' as message;

-- =====================================================
-- NEXT STEPS:
-- 1. Ensure your prm_members table has valid data
-- 2. Test the system by creating sample tasks
-- 3. Verify RLS policies work with your authentication
-- 4. Configure your frontend to use the new ID-based system
-- =====================================================